...
designBuildDirective.directive('drawDesign', function () {
  return {
    ...
    template: '<div class="db-image body sprite-body {{model}}"></div>' + '<div draw-dress parts="design.design"></div>' + '<div draw-accessories parts="design.extras"></div>'
  };
});
designBuildDirective.directive('drawDress', function () {
  return {
    scope: { parts: '=' },
    template: '<div ng-repeat="selection in parts">' + ' <div class="db-image {{selection.type}} sprite-{{selection.fabric}} {{selection.type}}-{{selection.id}}-{{selection.size}}-{{selection.fabric}}"></div>' + ' <div class="db-image trm sprite-{{selection.trim}} trm-{{selection.type}}-{{selection.id}}-{{selection.size}}-{{selection.trim}}"></div>' + '</div>'
  };
});
designBuildDirective.directive('drawAccessories', function () {
  return {
    scope: { parts: '=' },
    template: '<div ng-repeat="selection in parts">' + ' <div class="db-image {{selection.type}} sprite-{{selection.fabric}} {{selection.type}}-{{selection.id}}-{{selection.size}}-{{selection.fabric}}"></div>' + '</div>'
  };
});
