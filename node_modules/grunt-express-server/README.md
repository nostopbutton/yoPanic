...
grunt.initConfig({
  express: {
    ...
    dev: {
      options: {
        script: 'path/to/dev/server.bak.js'
      }
    },
    prod: {
      options: {
        script: 'path/to/prod/server.bak.js',
        node_env: 'production'
      }
    },
    test: {
      options: {
        script: 'path/to/test/server.bak.js'
      }
    }
  }
});
...
```js
  express: {
    options: {
      ...
      // Will turn into: `node OPT1 OPT2 ... OPTN path/to/server.bak.js ARG1 ARG2 ... ARGN`
      // (e.g. opts: ['node_modules/coffee-script/bin/coffee'] will correctly parse coffee-script)
      opts: [ ],
      args: [ ],    

      // Setting to `false` will effectively just run `node path/to/server.bak.js`
      background: true,
